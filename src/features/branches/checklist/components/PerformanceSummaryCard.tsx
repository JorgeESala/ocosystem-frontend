...
export default function PerformanceSummaryCard({ summary, loading }: PerformanceSummaryCardProps) {
    ...
    <Card className="border-slate-700/80 bg-slate-950/70">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-5">
          <div className={`flex h-24 w-24 shrink-0 items-center justify-center rounded-2xl text-3xl font-bold ${ringClass}`}>
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-300">
              {summary?.evaluableBranches ?? 0} de {summary?.totalBranches ?? 0} sucursales con datos
            </p>
        ...
        <div className="flex flex-wrap gap-2">
          {summary?.metrics?.map((m) => <MetricChip key={m.id} metric={m} />)}
        </div>
      </div>
      {summary?.metrics?.some((m) => m.evaluableBranches === 0) && (
        <p className="text-[11px] text-amber-300">
          Algunos indicadores no tienen datos en el periodo; quedan excluidos del puntaje general.
        </p>
      )}
    </Card>
  );
