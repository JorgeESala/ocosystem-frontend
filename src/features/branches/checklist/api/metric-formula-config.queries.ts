...
export const useUpdateMetricFormulaConfig = () => {
  ...
  return useMutation({
    mutationFn: ({ metricId, parameters }: { metricId: string; parameters: Record<string, number> }) =>
      metricFormulaConfigApi.update(metricId, parameters),
    onSuccess: (data: MetricFormulaConfig) => {
      queryClient.setQueryData(metricFormulaConfigKeys.all, (old: MetricFormulaConfig[] | undefined) => {
        if (!old) return [data];
