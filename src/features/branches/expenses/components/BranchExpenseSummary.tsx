...
export default function BranchExpenseSummary({
  totalAmount,
  expenseCount,
  averageAmount,
  scopeLabel,
    ...
    <section className="rounded-3xl border border-slate-700/80 bg-slate-950/70 p-5">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Stat
          ...
          label="Lidera"
          value={topBranch ?? "Sin datos"}
          hint={
            topExpenseCategory
              ? `Mayor tipo: ${topExpenseCategory}`
              : topBusinessUnit
                ? `Mayor unidad: ${topBusinessUnit}`
                : "Ninguna categoria con datos"
          }
        />
      </div>
    </section>
